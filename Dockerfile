...
# 暴露端口（EXPOSE 仅为文档/元数据，不影响运行时端口映射）
EXPOSE 10000

# 健康检查：优先检测含有上下文路径的 Actuator，其次回退到无上下文路径与自定义健康端点
HEALTHCHECK --interval=30s --timeout=10s --start-period=60s --retries=3 \
    CMD sh -c "curl -fsS http://localhost:$PORT/api/actuator/health || curl -fsS http://localhost:$PORT/actuator/health || curl -fsS http://localhost:$PORT/api/health || exit 1"
